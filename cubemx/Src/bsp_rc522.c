...
uint8_t rc522_read(int fd,void*buf,int count)
{
    ...
    if(count==0){
        ...
        else if((ucStatusReturn== MI_OK)&&(count==0)){
            if(*pTagType!=0x04){ //不是M1卡
                return MI_ERR;
            }
            if ( PcdAnticoll ( ucArray_ID ) == MI_OK ){
                if(PcdSelect(ucArray_ID)== MI_OK ){
                    if(PcdAuthState ( 0x61, RECORD_ADDR, rc522_KeyB, ucArray_ID )== MI_OK){
...
uint8_t SPI_RC522_ReadByte(void)
{
  ...
  if(HAL_SPI_TransmitReceive(&RC522spi,&d_send,&d_read,1,0xFFFFFF)!=HAL_OK)
    d_read=Dummy_Byte;
